...
def test_data_model_functions(fields):
    ...
    aliases = [fields.field_name, fields['field_name'], fields.get('field_name')]
...
def test_generate_field_default():
    ...
    assert Field(name="field_name", default=lambda: True).generate_default() is True

def test_get_is_visible():
    obj = {'some_val': None}
    assert Field(name="field_name").get_is_visible(obj) == True
    assert Field(name="field_name", is_visible=False).get_is_visible(obj) is False
    assert Field(name="field_name", is_visible=lambda obj: obj['some_val']).get_is_visible(obj) is None
